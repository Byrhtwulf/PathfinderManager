...
MonsterService.service('MonsterManager', function(){
    ...
    this.createMonsterFromDatabase = function(){
       ...
       "+1 keen longsword +8/+3 (1d8+6/17-20 plus infernal wound), +1 defending longsword +7 (1d8+2/19-20 plus infernal wound), bite +5 (1d8+2), 2 tentacles +5 (1d6+2)"
        ...
        newMonster.CMB = 0;
        newMonster.spellLikeAbilities = [];
        newMonster.spellsKnown = [];
        newMonster.spellsPrepared = [];
    ...
    this.generateMonsterAttacks = function(monsterFromDatabase, newMonster){
        ...
        for(var i = 0; i < rawAttackGroups.length; i++){
            ...
            for(var j = 0; j < attacks.length;j++) {
                ...
                var attackStringCopy = attackString;
                for (var l = 0; l < attackCount; l++) {
                    attackString = angular.copy(attackStringCopy);
    ...
    this.monstersFromDatabase = [
        {
            ...
            Speed_Mods: "",
            Melee: "2 claws +11 (1d3+2), bite +11 (1d4+2) or torturous touch +11 touch (2d6 plus 1d6 Dex plus pain)",
